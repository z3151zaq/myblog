...
async/await: without blocking concurrent. 传染性. concurrent proccessing
...
### Type And Key/Interface words in C#

#### dispose, finalizer, with(using)
- **Dispose**: Used in the `IDisposable` interface to release unmanaged resources explicitly. Implement `Dispose()` method to clean up resources.
- using statement automatically calls `Dispose()` on the object when it goes out of scope, ensuring resources are released promptly.
- finalizer: A special method (`~ClassName()`) that is called by the garbage collector before reclaiming the memory of an object. It’s used to clean up unmanaged resources if `Dispose()` was not called.
  - Finalizers are less efficient than `Dispose()` because they run at an indeterminate time, leading to delayed resource release.
  - Use finalizers only when necessary, and prefer implementing `IDisposable` for deterministic cleanup.
- **With**: Introduced in C# 9.0, it allows you to create a new instance of a record type with some properties modified, while keeping the rest unchanged. It’s a concise way to clone and modify records.

#### What is the difference between Task, async/await, and Thread?
Thread	Low-level manual thread	✅ Yes	✅ Usually	Background computation, parallel work
Task	High-level unit of work	✅ Maybe (or pooled)	❌ With await	Asynchronous or parallel tasks
async/await: Syntactic sugar for async code	🚫 Not directly	❌ No	I/O-bound async (file, HTTP, DB)

#### What is the use of the async and await keywords?（important）
In C#, the async and await keywords are used together to simplify asynchronous programming, making it easier to write code that does non-blocking work—like I/O, web requests, or database queries—without freezing the main thread.
...
#### What is extension method in C#?（important）
...
| **Type Safety**          | ✅ Yes (strong typing)         | ❌ No (errors at runtime possible)  | ✅ Yes (with casting)               |
| **Requires Casting?**    | ❌ No                          | ❌ No                               | ✅ Yes (to access specific members) |
| **IntelliSense Support** | ✅ Yes                         | ❌ No                               | ✅ Yes (after cast)                 |
...
#### What is the difference between IEnumerable and IQueryable in LINQ?（important）
...
| ------------------------------- | ----------------------------- | ---------------------- | ----------------------- | --------------------------------------- |
| **Reference Type?**             | ✅ Yes                        | ✅ Yes                 | ❌ No (value type)      | ❌ No (value type)                      |
| **Stored in Heap or Stack?**    | 🔴 Heap                       | 🔴 Heap                | ✅ Stack                | ✅ Stack                                |
| **Default Mutability?**         | ✅ Mutable                    | ❌ Immutable           | ✅ Mutable              | ❌ Immutable                            |
| **Comparison Type**             | 🔴 Reference Equality         | ✅ Value Equality      | ✅ Value Equality       | ✅ Value Equality                       |
| **Supports Inheritance?**       | ✅ Yes                        | ✅ Yes                 | ❌ No                   | ❌ No                                   |
| **Supports `with` Expression?** | ❌ No                         | ✅ Yes                 | ❌ No                   | ✅ Yes                                  |
...
Middleware is a software component in the ASP.NET Core request pipeline that can inspect, modify, or handle HTTP requests and responses. It’s used for cross-cutting concerns like authentication, logging, routing, exception handling, etc.
...
you register services in the Startup.cs file using Dependency Injection (DI) via the ConfigureServices method.
- AddTransient
- AddScoped
- AddSingleton
...
Model Binding is the process where ASP.NET Core automatically maps HTTP request data (from query strings, forms, route values, headers, JSON bodies, etc.) to your C# method parameters or objects. We don’t need to parse things manually — the framework does it for you.
...
The request pipeline in ASP.NET Core is a sequence of middleware components that process HTTP requests and responses. It’s designed to be **modular, lightweight, and customizable**, allowing you to control how requests are handled and responses are generated.
...
- Transient: Every time requested Never reused
  ...
  - You don’t need to share it across requests or components.
...
- CLR uses the JIT compiler to convert CIL → native machine code excuted by cpu
...
- Easier Unit Testing – Dependencies can be replaced with mock objects.
- Better Code Maintainability – Changing one component doesn't affect others.
- Improved Scalability – Easy to extend functionality without modifying existing code.
...
- Generation 0 (Short-lived objects) – Objects that are created and removed quickly.
- Generation 1 (Medium-lived objects) – Objects that survive Generation 0 collection.
- Generation 2 (Long-lived objects) – Objects that persist for a long time, like static data.
...
.NET Standard is a specification – not a runtime – that ensures code compatibility across different .NET platforms.

#### How do you return a file download in a Web API controller?

In ASP.NET Core Web API, to return a file download from a controller, you typically use the built-in helper methods like File() inside your controller action.
- File(byte[], type, name)
- File(Stream, type, name)

#### How do you implement logging in .NET Core?
logging is built-in via the Microsoft.Extensions.Logging namespace. It’s flexible, supports dependency injection (DI), and integrates with various providers like Console, Debug, File, Application Insights, Serilog, etc.
...
SignalR is a real-time communication library over WebSocket for ASP.NET Core. It lets your server push updates to connected clients instantly—like chat messages, notifications, or live data updates—without the client needing to refresh or poll.
...
Kestrel is the cross-platform web server that’s built into ASP.NET Core.
